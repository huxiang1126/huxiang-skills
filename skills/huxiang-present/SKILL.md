...
| `#+author:` 或 `#+date:` | → JSON `subtitle`（页脚右下，覆盖默认的 `@虎小象`） |
| `#+filetags:` | 用于推断 theme（见下） |
| `#+identifier:` | 忽略 |

**默认 subtitle = `@虎小象`**（无 `#+author:`/`#+date:` 时）。页脚同时显示头像（`assets/avatar.jpg` 内联为 base64 data URL），头像为 28px 圆形，与文字同行。
...
4. **应用映射规则**生成 slides 数组：
   ...
   - 强调 → 自动 hl
   - example 块 → 独立 pre 页
5. **Read** `assets/slogan_template.html`（cyber 主题需在模板基础上注入扫描线/HUD/光标 CSS）；同时读取 `assets/avatar.jpg` 作为字节数据，base64 编码为 `data:image/jpeg;base64,...`
6. **替换占位符**：
   - `{{TITLE}}` → 文件 `#+title:` 或显式参数
   - `{{SUBTITLE}}` → `#+author:` `#+date:` 拼接，**无则默认 `@虎小象`**
   - `{{THEME}}` → 推断或显式参数（black|red|yellow|cyber）
   - `{{SLIDES_JSON}}` → JSON.stringify(slides)
   - `{{AVATAR_DATA_URL}}` → `assets/avatar.jpg` 的 base64 data URL（让输出 HTML 单文件自包含）
