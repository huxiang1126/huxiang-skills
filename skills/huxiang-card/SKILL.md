...
```bash
node ~/.claude/skills/huxiang-card/assets/capture.js <html> <png> <width> <height> [fullpage]
```

依赖：`~/.claude/skills/huxiang-card/node_modules/` 中的 playwright。如报错：

```bash
cd ~/.claude/skills/huxiang-card && npm install playwright && npx playwright install chromium
```

### Footer

- 左侧：logo + 虎小象（已硬编码在模板中）
